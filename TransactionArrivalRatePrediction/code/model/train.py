...
def main():
    ...
    # Loading Dataset
    train_data = LoadData(data_path='../data/transaction_send_rates/ReserveSendRates_1minute.npy', divide_days=[24, 6],
                          time_interval=1, history_length=12, train_mode='train')
    train_loader = DataLoader(train_data, batch_size=64, shuffle=True, num_workers=32)

    test_data = LoadData(data_path='../data/transaction_send_rates/ReserveSendRates_1minute.npy', divide_days=[24, 6],
                         time_interval=1, history_length=12, train_mode='test')
    test_loader = DataLoader(test_data, batch_size=64, shuffle=False, num_workers=32)

    # Loading Model
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    device = torch.device('cpu')
    # my_net = TemporalConvNet(num_inputs=1, num_channels=[32, 8, 1], kernel_size=3, dropout=0.2).to(device)
    my_net = RNN(device=device, rnn_type='LSTM', input_size=1, hidden_size=128,
                 num_layers=2, dropout=0, bidirectional=False).to(device)
    ...
    for epoch in range(Adam_epochs):
        ...
        print('Epoch: {:04d}, Loss: {:02.4f}, mae: {:02.4f}, '
              'rmse: {:02.4f}, mape: {:02.4f}, Time: {:02.2f} s'.format(epoch + 1,
                                                                           10 * epoch_loss / (len(train_data) / 64),
                                                                           epoch_mae,
                                                                           epoch_rmse,
                                                                           epoch_mape,
                                                                           end_time - start_time))
    print("average training time (s / epoch): " + str(np.mean(time_list)))
    # Test Model
    my_net.eval()
    with torch.no_grad():
        epoch_mae = 0.0
        ...
        print('Test Loss: {:02.4f}, mae: {:02.4f}, rmse: {:02.4f}, mape: {:02.4f}, Time: {:02.2f} s'.format(10 * total_loss / (len(test_data) / 64),
                                                                                          epoch_mae,
                                                                                          epoch_rmse,
                                                                                          epoch_mape,
                                                                                          end_time - start_time))
    ...
    y_true = test_data.recover_data(test_data.park_norm[0], test_data.park_norm[1], all_y_true).numpy().reshape(8640,)
    y_pred = test_data.recover_data(test_data.park_norm[0], test_data.park_norm[1], all_predict_value).numpy().reshape(8640,)
    np.save("../result/y_true_lstm_1min.npy", y_true)
    np.save("../result/y_pred_lstm_1min.npy", y_pred)
