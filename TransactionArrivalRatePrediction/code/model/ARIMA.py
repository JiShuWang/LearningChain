...
def main():
    ...
    # print(testx.shape)  # (576, 12, 1)
    # print(testy.shape)  # (576, 1, 1)
    y_pred = arima(testx)
    print(y_pred.shape)
    np.save('../result/y_pred_arima_1min.npy', y_pred)
    np.save('../result/y_true_arima_1min.npy', testy)
    y_true = pd.Series(np.load('../result/y_true_arima_1mins.npy').reshape(8640, ), name='y_true')
    y_pred = pd.Series(np.load('../result/y_pred_arima_1mins.npy').reshape(8640, ), name='y_pred')

    y = pd.concat([y_true, y_pred], axis=1)
    y.to_csv('../result/y_true_pred_arima_1min.csv')
    print(y)
